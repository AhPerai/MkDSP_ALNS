...
class street_network_env:
    def __init__(self, coord):
        ...
        print("+++++++++++++++++")
        self.sn = osmnx.graph_from_point(
            center_point=(self.cox, self.coy),
            dist=sn_radius,
            network_type="drive",
            dist_type="bbox",
        )
        ...
        for i in self.sn.nodes:
            en = networkx.ego_graph(
                self.sn, i, distance="length", radius=reachability_radius
            )
        ...
        corcoran2021.utilities.valid_undirected_adjacency_list(self.al)
...
if __name__ == "__main__":
    ...
    for key in cites:
        ...
        with open(file_name, "w") as f:
            f.write(
                str(st_obj.rn.number_of_nodes())
                + " "
                + str(st_obj.rn.number_of_edges())
            )
