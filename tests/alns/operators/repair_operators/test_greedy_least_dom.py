...
from tests.utils.state_info_assertions import assert_state_equal, init_state_k_only
...
@pytest.mark.parametrize("operator_class", [GreedyLeastDominatedOperator])
@pytest.mark.parametrize(
    "instance_path,K", [("instances/cities_small_instances/york.txt", 2)]
)
def test_greedy_least_dom_operator_generate_valid_solution(
...
def test_greedy_least_dom_operator_dominates_graph(operator_class, instance_path, K):
...
def test_update_state_consistency(iterations):
    ...
    degree_repair_op = GreedyLeastDominatedOperator()
    ...
    for i in range(iterations):
        ...
        if rng.random() < 0.7:
            ...
            S_expected = init_state_k_only(S_destroyed)
            ...
            S_expected = init_state_k_only(S_modified)
